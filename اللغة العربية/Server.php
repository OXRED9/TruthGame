...
if (isset($_POST['sendBtn'])) {
   if (empty(trim($_POST['firstQu'])) || empty(trim($_POST['secondeQu'])) || empty(trim($_POST['thiredQu']))) {
      $_SESSION['error'] = "الرجاء كتابة جميع الاسئلة";
      ...
      if ($result) {
         $_SESSION['done'] = "تم إرسال اسئلتك ياوحش";
...
if (isset($_POST['adminBtn'])) {
   if (empty(trim($_POST['username'])) || empty(trim($_POST['password']))) {
      $_SESSION['adminMsg'] = "الرجاء ملء جميع الحقول";
      ...
      if ($result->num_rows > 0) {
         ...
         $_SESSION['adminMsg'] = "اسم المستخدم او كلمة المرور خاطئة";
         header('location: /Truth/admin.php?login');
      }
   }
}
